...
**Context:** `ProjectConfig` is at line 28. `DependabotConfig` is at line 177. No test runner exists — use `pnpm tsc --noEmit` from `/home/user/Projects/hexops` for verification.
...
```bash
cd /home/user/Projects/hexops && pnpm tsc --noEmit
```
...
```bash
cd /home/user/Projects/hexops && git commit src/lib/types.ts -m "feat(branch-prop): add BranchSyncStatus and PropagationConfig types (#71)"
```
...
```bash
cd /home/user/Projects/hexops && git commit src/lib/github-client.ts -m "feat(branch-prop): add openPropagationPR to github-client (#71)"
```
...
```bash
cd /home/user/Projects/hexops && git add src/lib/branch-propagator.ts && git commit -m "feat(branch-prop): add branch-propagator lib (#71)"
```
...
```bash
cd /home/user/Projects/hexops && git add src/app/api/projects/\[id\]/branch-sync/ && git commit -m "feat(branch-prop): add GET branch-sync API route (#71)"
```
...
```bash
cd /home/user/Projects/hexops && git add src/app/api/projects/\[id\]/propagate-branches/ && git commit -m "feat(branch-prop): add POST propagate-branches API route (#71)"
```
...
```bash
cd /home/user/Projects/hexops && git add src/components/branch-propagate-modal.tsx && git commit -m "feat(branch-prop): add BranchPropagateModal component (#71)"
```
...
```bash
cd /home/user/Projects/hexops && git commit src/components/detail-sections/dependabot-panel.tsx -m "feat(branch-prop): add sync warning banner and propagate button to DependabotPanel (#71)"
```
...
```bash
cd /home/user/Projects/hexops && git push origin feature/dependabot-integration
```
...
```bash
cd /home/user/Projects/hexops && gh api graphql -f query='
mutation {
